...
class PromptTokenStats:
    ...
    def update_from_output(
        ...
        # Defensive sanitization: metrics counters cannot be incremented by
        # negative values. In practice, scheduler/model-runner bugs or custom
        # scheduling policies may temporarily violate the expected invariants.
        # Clamp to a safe range to keep the engine running.
        if prompt_len < 0:
            prompt_len = 0
        ...
        # num_external_computed_tokens should not exceed the total cached tokens
        # (including the recomputed token below). We will clamp after recomputed
        # is computed.
        ...
        max_external = num_cached_tokens + recomputed
        if num_external_computed_tokens > max_external:
            num_external_computed_tokens = max_external

        computed_inc = prompt_len - num_cached_tokens
        if computed_inc < 0:
            computed_inc = 0
        self.computed += computed_inc
 ...
 # ================================
        local_hit_inc = num_cached_tokens + recomputed - num_external_computed_tokens
        if local_hit_inc < 0:
            local_hit_inc = 0
        self.local_cache_hit += local_hit_inc
 # ================================
