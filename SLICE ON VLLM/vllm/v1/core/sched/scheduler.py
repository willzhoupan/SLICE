...
from vllm.v1.core.sched.priority_mapper import optimize_job_priority, job_from_dict, job_to_dict,Job, JobType, SLO
from vllm.v1.core.sched.priority_mapper import SimulatedAnnealingPriorityMapper

logger = init_logger(__name__)


def _reorder_running_by_optimized_jobs(
    running: list[Request],
    optimized_jobs: list[dict[str, Any]],
) -> None:
    """按 ``optimized_jobs`` 中 ``input_length``（对应 ``request_id``）顺序原地重排 ``running``。"""
    if not running or not optimized_jobs:
        return
    running_ids = {r.request_id for r in running}
    id_to_req = {r.request_id: r for r in running}
    ordered_ids: list[str] = []
    seen_ids: set[str] = set()
    for job in optimized_jobs:
        rid = str(job["input_length"])
        if rid in running_ids and rid not in seen_ids:
            ordered_ids.append(rid)
            seen_ids.add(rid)
    for req in running:
        if req.request_id not in seen_ids:
            ordered_ids.append(req.request_id)
            seen_ids.add(req.request_id)
    running[:] = [id_to_req[i] for i in ordered_ids]
...
class Scheduler(SchedulerInterface):
    def __init__(
        ...
        # =========================
        # SLICE scheduling state
        # =========================
        self.matrix_select_queue: list[Request] = []
        # Requests admitted for matrix construction / rate control.
        self.matrix_pending_queue: list[Request] = []
        # N x 50 matrix: row[0] is request_id, row[1:50] are 0/1 masks.
        self.slice_matrix: list[list[Any]] = []
        # Column scan index in [1, 49]. (0 is reserved for request_id)
        self.slice_matrix_index: int = 1
        # Effective number of data columns in SLICE matrix.
        # Use at least 49 to preserve original behavior, and scale with
        # max_num_running_reqs so batch>50 has enough control resolution.
        self.slice_data_cols: int = max(49, self.max_num_running_reqs)
        # Whether we need to rebuild the matrix on next schedule() call.
        self.need_reconstruct: bool = True
        # Track active request membership; SLICE rebuilds only on arrival/leave.
    ...
    def _construct_matrix(self):
        ...
        # 1. 重置矩阵状态
        self.slice_matrix = [] # 对应 dynamicMatrix
        self.slice_matrix_index = 1 # 对应 dynamicMatrixIndex = 1 (注意C++是从1开始的)
    
        if not self.matrix_pending_queue:
            return
    
        # 2. 准备带 Ratio 的任务列表并按 1000/TPOT 降序排序
        # task_list 元素结构: (request_id, fill_count)
        task_list = []
        for req in self.matrix_pending_queue:
            ratio = 1000.0 / getattr(req, 'tpot_target', 50.0)
            task_list.append((req.request_id, int(ratio)))
    
        # 降序排序：Ratio 越大（TPOT 越短）的任务排在矩阵越靠上的行
        task_list.sort(key=lambda x: x[1], reverse=True)
    
        # 3. 构建矩阵 (N x 50)
        for i in range(len(task_list)):
            request_id, fill_count = task_list[i]
            
            # 创建新行，初始化为 50 个 0
            row = [0] * 50
            
            # 第一列存储任务 ID (对应 dynamicMatrix[i][0] = taskHandleId)
            row[0] = request_id
    ...
    def schedule(self) -> SchedulerOutput:
        ...
        if self.need_reconstruct:
            ...
            for req in all_active_requests:
                tpot_val = float(getattr(req, "tpot_target", 50.0))
                tpot_ms = tpot_val * 1000.0 if tpot_val < 1.0 else tpot_val
                tpot_ms = max(tpot_ms, 1e-6)
                tasks_with_ratio.append((req, 1000.0 / tpot_ms))


            #任务选择
            max_n = min(
                len(tasks_with_ratio),
                len(l_values),
            ...
            for n in range(1, max_n + 1):
                ...
                for i in range(1, n + 1):
                    tpot_i_ratio = tasks_with_ratio[i - 1][1]
                    
                    safe_l_idx = i - 1
    
                    if i == n:
                        
                        f_n += tpot_i_ratio * l_values[safe_l_idx]
                    else:
                        
                        tpot_next_ratio = tasks_with_ratio[i][1] 
                        f_n += (tpot_i_ratio - tpot_next_ratio) * l_values[safe_l_idx]
    
                
                if f_n <= 1000:
                    self.matrix_pending_queue.append(tasks_with_ratio[n - 1][0])
                else:
                    break
        ...
        #基于矩阵列扫描的速率控制
        current_col = self.slice_matrix_index
        self.matrix_select_queue.clear()
        

        # 扫描矩阵行
        for row in self.slice_matrix:
            req_id = row[0]
            should_run = (row[current_col] == 1)
            if should_run:
                request = self.requests.get(req_id)
                self.matrix_select_queue.append(request)
        ...
        self.running.sort(key=lambda r: r.reward, reverse=True)
        req_index = 0
        while req_index < len(self.running) and token_budget > 0:
            ...
            # 筛选不是矩阵选择的任务
            if request not in self.matrix_select_queue:
                break
        ...
        if not preempted_reqs and self._pause_state == PauseState.UNPAUSED:
            ...
            while req_index_waiting < len(self.matrix_select_queue) and token_budget > 0:

                
                
                request = self.matrix_select_queue[req_index_waiting]
                request_id = request.request_id
                ...
                while True:
                    ...
                    # Find the request with minimum reward in running queue
                    if self.running:
                        preempted_req = self.running[-1]
                        if request.reward > preempted_req.reward:
    ...
    def add_request(self, request: Request) -> None:
        ...
        self.need_reconstruct = True # 标记需要重构矩阵
    ...
    def shutdown(self) -> None:
        if self._slice_priority_optimize_invocations > 0:
            total_s = self._slice_priority_optimize_total_s
            n = self._slice_priority_optimize_invocations
            logger.info(
                "[Huawei] priority optimize (build jobs + optimize + reorder): "
                "total %.6f s, %d invocations, avg %.3f ms per invocation",
                total_s,
                n,
                1000.0 * total_s / n,
            )
