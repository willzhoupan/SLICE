...
class Request:
    def __init__(
        ...
        e2e: float = 0,
        meetSLO: float = 0,
        meetSLO_ttft: float = 0,
        meetSLO_tpot: float = 0,
        demotion_reward: bool = False,
        ...
        self.preoutput_len = preoutput_len
        self.predecoding = predecoding
        self.preTTF = preTTF
        self.e2e = self.preTTF + self.predecoding
        self.meetSLO = meetSLO
        self.meetSLO_ttft = meetSLO_ttft
        self.meetSLO_tpot = meetSLO_tpot
        self.demotion_reward = False
    ...
    def from_engine_core_request(
        ...
        # Custom scheduler metadata (e.g. SLICE) can be passed via
        # sampling_params.extra_args from the API layer.
        ...
        tpot_target = float(extra_args.get("tpot_target", 50.0))
        u_value = float(extra_args.get("u_value", 1.0))
        reward = float(extra_args.get("reward", tpot_target * u_value))
        preoutput_len = int(extra_args.get("preoutput_len", 0))
        ...
        return cls(
            ...
            # --- [SLICE 核心修改：确保从传输对象里读出这三个值] ---
            reward=getattr(request, 'reward', 0.0),
            tpot_target=getattr(request, 'tpot_target', 50.0),
            u_value=getattr(request, 'u_value', 0.0),
            preoutput_len=getattr(request, 'preoutput_len', 0),
