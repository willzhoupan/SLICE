...
namespace fastllm {
    ...
    struct GenerationConfig {
        ...
        bool enableStarvation = false; // 是否启用饥饿提升
        double starvation_threshold = 0.0; // 饥饿提升阈值
        
        
        // 添加参数存储映射，用于自定义参数
        std::map<std::string, int> mapIntParams;
        std::map<std::string, float> mapFloatParams;
        std::map<std::string, std::string> mapStringParams;

        // 检查是否包含特定参数
        bool contains(const std::string &key) const {
            return mapIntParams.find(key) != mapIntParams.end() ||
                   mapFloatParams.find(key) != mapFloatParams.end() ||
                   mapStringParams.find(key) != mapStringParams.end();
        }

        // 获取特定整数参数值
        int at(const std::string &key) const {
            auto it = mapIntParams.find(key);
            if (it != mapIntParams.end()) {
                return it->second;
            }
            // 如果找不到，返回0
            return 0;
